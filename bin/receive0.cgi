...
cmd_template=Template("cd $outdir;/usr/bin/timeout 1m $TMalign -mm ${mm} ${jobID}A.pdb ${jobID}B.pdb -byresi ${byresi} -mol ${mol} -atom \"${atom}\" -o ${jobID}super")
...
def get_cgi_field():
    ...
    pdbA=form.getfirst("pdb01_txt",'')
    pdbB=form.getfirst("pdb02_txt",'')
    if not pdbA.strip():
        pdbA=form.getfirst("pdb01_file",'').strip()
        if len(pdbA):
            pdbA=pdbA.decode()
    if not pdbB.strip():
        pdbB=form.getfirst("pdb02_file",'').strip()
        if len(pdbB):
            pdbB=pdbB.decode()
    byresi=form.getfirst("byresi","0")
    mol=form.getfirst("mol","auto")
    atom=form.getfirst("atom","auto")
    mm=form.getfirst("mm","0")
    return str(pdbA),str(pdbB),byresi,mol,atom,mm
...
if __name__=="__main__":
    ...
    pdbA,pdbB,byresi,mol,atom,mm=get_cgi_field()
    ...
    cmd=cmd_template.substitute(outdir=outdir,TMalign=TMalign,jobID=jobID,
        byresi=byresi,mol=mol,atom=atom,mm=mm)
